  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative min-h-screen flex items-center px-4 md:px-8 lg:px-16 pt-16 overflow-hidden">
        ...
        {/* Gradiente blanco más suave para tono piel claro */}
      ...
      <div className="relative py-3 md:py-4 overflow-hidden bg-[#c05264]">
        <div className="relative z-10 flex items-center justify-center gap-4 md:gap-8 animate-marquee whitespace-nowrap">
          {Array(30).fill("NEW").map((item, index) => (
            <span key={index} className="text-white/90 mx-2 md:mx-4 text-xs md:text-sm tracking-wider font-semibold inline-block">
              ✦ {item} ✦
            </span>
          ))}
        </div>
      ...
      <section className="py-12 md:py-20 px-4 md:px-8 lg:px-16 bg-[#fef7f2]">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-8 md:mb-12">
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-light tracking-tight text-gray-600">Vitamin</h2>
            <div className="w-12 md:w-16 h-px bg-[#c05264]/30 mx-auto my-3 md:my-4" />
            <p className="text-gray-400 text-sm md:text-base">Discover our premium collection</p>
          </div>
          ...
          {loading && (
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-gray-500 text-sm md:text-base">Cargando productos...</p>
            </div>
          )}
          ...
          {!loading && featuredProducts.length > 0 && (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-6">
              {featuredProducts.map((product: any) => (
                <Link href={`/productos/${product.slug}`} key={product.id} className="group">
                    ...
                    <div className="aspect-square bg-gradient-to-br from-amber-50 to-[#fef2f0] rounded-lg md:rounded-xl mb-2 md:mb-4 flex items-center justify-center group-hover:scale-105 transition-transform duration-300 shadow-sm hover:shadow-md overflow-hidden relative">
                      {product.imageUrl ? (
                        ...
                        <div className="text-center">
                          ...
                          <span className="text-xs text-gray-500 hidden md:block">{product.name}</span>
                        </div>
                      )}
                    </div>
                    <h3 className="font-medium text-xs md:text-sm mb-0.5 md:mb-1 text-gray-600 line-clamp-1">{product.name}</h3>
                    <p className="text-[#c05264] text-xs md:text-sm font-medium">${product.price}</p>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <section className="py-12 md:py-20 px-4 md:px-8 lg:px-16 bg-[#fef7f2]">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-light tracking-tight text-gray-600 mb-3 md:mb-4">What we offer</h2>
